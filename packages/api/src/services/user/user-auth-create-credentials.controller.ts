...
} from "common/lib/contracts/auth"
...
const validateServiceResponse = (response?: Authentication | null) => {
	if (!response || !response?.email) return unauthorized()
...
export const createAuthCredentialsController =
	...
	): Controller<CreateAuthCredentialsContract, Authentication> =>
	async (request) => {
